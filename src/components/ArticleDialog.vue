...
const handleSubmit = async () => {
    try {
        ...
        if (!isEdit.value) {
            ...
            res = await updateArticle(props.article.id, submitData)
            console.log('✅ 更新成功:', res)
        }
        
        // 5. 👇 关键：延迟触发刷新，确保后端数据已写入
        setTimeout(() => {
            emit('success')
        ...
        if (err.fields) {
            // 表单验证失败
            const firstError = Object.values(err.fields)[0]?.[0]?.message || '表单验证失败'
