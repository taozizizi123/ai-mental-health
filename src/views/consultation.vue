                ...
                <div class="warm-tips">
                    ...
                    <div class="emotion-intensity">
                        <span class="intensity-dots">
                            <span v-for="dot in 3" :key="dot" class="{active: getIntensityClass(currentEmotion.emotionScore) >= dot}"></span>
                        </span>
                ...
                <div class="message-item ai-message" v-if="messages.length === 0">
                    ...
                    <div class="message-content">
                        <div class="message-bubble">
                            <p>您好！我是小暖，您的AI心理健康助手，很高兴陪伴您，为您提供温暖的心理支持，请告诉我，今年您感觉怎么样？有什么想要分享的吗？</p>
                        </div>
...
// 👇 在 script setup 顶部添加模拟开关
const USE_MOCK_AI = true  // 🔧 true=使用模拟回复，false=使用真实接口

//新建会话
// 👇 完全替换原来的函数
const createNewFrontendSession = () => {
    console.log('🔘 点击了新建会话')  // 👈 加调试日志
...
const loadSessionEmotion = (sessionId) => {
    //确保sessionID格式正确
    const id = sessionId.toString().startsWith('session-') ? sessionId : `session-${sessionId}`
...
const startAIResponse = (sessionId, usermessage) => {
    ...
    const typingTimer = setInterval(() => {
        if (index < mockReply.length) {
            aiMessage.content += mockReply.charAt(index)
            index++
            ...
            isAiTyping.value = false  // 👈 关键：解锁输入框
...
// 👇 新增：根据用户输入返回模拟回复
const getMockReply = (userMessage) => {
    const lowerMsg = userMessage.toLowerCase()
    
    if (lowerMsg.includes('你好') || lowerMsg.includes('嗨')) {
        return '你好！很高兴见到你😊 我是宁渡AI心理健康助手，有什么想聊的吗？'
    }
    if (lowerMsg.includes('开心') || lowerMsg.includes('高兴')) {
        return '太好了！听到你开心我也很高兴🎉\n\n保持好心情是很重要的，建议你可以：\n• 记录下这份快乐\n• 和朋友分享\n• 做点喜欢的事情奖励自己'
    }
    if (lowerMsg.includes('难过') || lowerMsg.includes('伤心')) {
        return '我能理解你现在的感受💔\n\n每个人都会有低落的时候，这很正常。你可以：\n• 深呼吸，放松一下\n• 听听喜欢的音乐\n• 或者和我聊聊发生了什么'
    }
    if (lowerMsg.includes('焦虑') || lowerMsg.includes('担心')) {
        return '我感受到你的不安，焦虑确实很难受😰\n\n试试这些方法：\n1. 深呼吸：吸气4秒，憋气7秒，呼气8秒\n2. 专注于当下\n3. 如果愿意，可以和我说说你在担心什么'
    }
    if (lowerMsg.includes('压力') || lowerMsg.includes('累')) {
        return '听起来你最近很辛苦😔\n\n记得照顾好自己：\n• 适当休息，不要硬撑\n• 分解任务，一步一步来\n• 找人倾诉\n\n你已经做得很好了！'
    }
    if (lowerMsg.includes('谢谢') || lowerMsg.includes('感谢')) {
        return '不客气😊 能帮到你我也很开心！\n\n记住，无论遇到什么困难，都要相信自己。你比想象中更强大！'
    }
    ...
    // 默认回复
    return '谢谢你的分享。我会一直在这里倾听你的心声。\n\n如果你想继续聊，我随时都在。记住，你并不孤单~'
}

// 👇 真实AI回复函数（保留原逻辑）
const realAIResponse = (sessionId, usermessage, aiMessage) => {
    ...
    fetchEventSource('/api/psychological-chat/stream', {
        ...
        onError: (err) => {
            ...
            // 如果真实接口失败，可以自动降级到模拟模式
            if (aiMessage.content === '') {
                aiMessage.content = '（后端服务暂时不可用，已切换到模拟模式）\n\n'
                simulateAIResponse(aiMessage, usermessage)
            } else {
...
const handleDeleteSession = (sessionId) => {
    deleteSession(sessionId).then(res => {
        ElMessage.success('删除成功')      //删除成功后，更新会话列表
        getSessionPage()
    })
}
//简单的换行逻辑
